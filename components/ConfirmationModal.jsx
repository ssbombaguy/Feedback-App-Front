      ...
      <View style={styles.overlay}>
        <View style={styles.modalContainer}>
          <View style={[styles.iconContainer, isDangerous && styles.dangerIcon]}>
            <MaterialIcons name={isDangerous ? 'warning' : 'help'} size={40} color="#fff" />
          </View>
          <Text style={styles.title}>{title}</Text>
          <Text style={styles.message}>{message}</Text>
          <View style={styles.buttonContainer}>
            <TouchableOpacity
              style={[styles.confirmButton, isDangerous && styles.dangerButton, isLoading && styles.buttonDisabled]}
              onPress={onConfirm}
              disabled={isLoading}
            >
